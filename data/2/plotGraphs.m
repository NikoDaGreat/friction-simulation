...
for currentForce = forces
    ...
    figure
    subplot(2,1,1)
    plot(time,y,'r.');hold on;plot(time,z,'k*');grid on;xlabel('Time (fs)');ylabel('Force (eV/Å)')
    legend('Y','Z'); title(strcat(strcat('Load ', txt), ' (eV/Å)'));
    
    
    subplot(2,1,2)
    plot(time,y,'r-');grid on;xlabel('Time (fs)');ylabel('Force (eV/Å)')
    legend('Y'); 
