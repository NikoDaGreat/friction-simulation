...
for currentForce = forces
    ...
    subplot(2,1,1)
    plot(time(1:floor(end/2)),temperature(1:floor(end/2)),'r.');grid on;xlabel('Time (fs)');ylabel('Temperature (K)')
    title(strcat(strcat('Load ', txt), ' (eV/Å)'));
    hold on;ylim([min(temperature) max(temperature(1:floor(end/2)))]);
    for index=1:ceil(n_lines/2)
        line([index*atom_point index*atom_point], [0 20000])
    end
    ...
    plot(time(1:floor(end/2)),sm_sm_temp(1:floor(end/2)),'r-');grid on;xlabel('Time (fs)');ylabel('Temperature (K)')
    hold on;ylim([min(temperature) max(temperature(1:floor(end/2)))]);
    for index=1:ceil(n_lines/2)
        line([index*atom_point index*atom_point], [0 20000])
    end
